...
| Source lifecycle | Rotation/delete/verification failure invalidates an active job, silently erases or invents a lease, rewrites old audit history, poisons a prior snapshot, or leaves a usable deleted secret | Immutable source and credential revisions; unified full-history state/lease replay from revision 1; append-only acquisition/release/cancel ledger; never-reused lease/job IDs; active snapshot and count rebuilt from ledger; admission/release/cancel must precede the same-revision state snapshot; drain admission preserves identities; each release/cancel removes one acquired identity; forced cancel requires prior Owner approval; operation matches pending operation; delete only at zero leases; metadata-only tombstone | Standalone-snapshot and prior/proposed tamper tests; concurrent rotation/disable/delete/verification failure; acquisition, entry-with-live-leases, identity mismatch, normal-release, forced-cancel, cross-ledger event-order, crash-recovery, verification-state, and tombstone tests |
...
| LLM output | Hallucination or prompt injection injects a destructive conclusion/action, fabricated measurement, non-hit rule, or self-declared evidence strength | Model selects only server-owned template IDs and typed parameters; numeric/object fields originate in eligible evidence-bound typed facts; restricted RFC 8785 digest rejects non-finite/fractional measurements; versioned rule registry owns predicates/status/severity/text/references; level/completeness/uncertainty are derived; full fact/decision/claim/action deterministic rendering; explicit degraded versus non-invoked abstention; labeled provider/model/prompt/redacted-payload digest/redaction provenance; no tool/execution path | Destructive SQL, low-signal/non-hit rule, stale/zero/truncated Evidence, self-raised level, NaN/Infinity, fabricated measurement, typed-payload/summary tampering, altered priority/evidence summary, wrong evidence kind, silent fallback, unknown template/pack, altered rendering, provenance omission, injection, and malformed-output corpus |
| Outcome audit | A terminal state combines unrelated approvals/results, uses an automated approval, or claims effect/rollback without qualifying Evidence | One singular Action/approval/implementation/result/terminal-feedback tuple; exact event projection; digest-checked server authorization snapshot for a real user; proposed-revision ownership; eligible effect Evidence whose metric code/unit and target are bound to the Action template; passed effect or confirmed rollback; causal timestamp order | Multiple-tuple, system-approval, authorization-digest, wrong-revision, wrong-metric, failed-effect/rollback, ineligible result, extra-result, and time-travel counterexamples |
...
- Typed Evidence canonicalization rejects NaN, Infinity, fractional
  measurements, and integers outside the cross-language safe range.
...
- All evidence frozen into the ready diagnosis exists no later than the ready
  ...
  Human approval is a real user bound to a server-owned authorization snapshot;
  terminal effect/rollback Evidence must be eligible and bound to the same
  Action validation target.
